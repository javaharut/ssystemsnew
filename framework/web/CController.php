...
class CController extends CBaseController
{
	...
	/**
	 * @var mixed the name of the layouts to be applied to this controller's views.
	 * Defaults to null, meaning the {@link CWebApplication::layouts application layouts}
	 * is used. If it is false, no layouts will be applied.
	 * The {@link CWebModule::layouts module layouts} will be used
	 * if the controller belongs to a module and this layouts property is null.
	 */
	public $layout;
	/**
	...
	/**
	 * Looks for the layouts view script based on the layouts name.
	 *
	 * The layouts name can be specified in one of the following ways:
	 *
	 * <ul>
	 * <li>layouts is false: returns false, meaning no layouts.</li>
	 * <li>layouts is null: the currently active module's layouts will be used. If there is no active module,
	 * the application's layouts will be used.</li>
	 * <li>a regular view name.</li>
	 * </ul>
	 *
	 * The resolution of the view file based on the layouts view is similar to that in {@link getViewFile}.
	 ...
	 * cannot be themed because they can refer to a view file located at arbitrary places.</li>
	 * <li>relative view: otherwise. Relative views will be searched for under the currently active
	 * module's layouts path. In case when there is no active module, the view will be searched for
	 * under the application's layouts path.</li>
	 * </ul>
	 *
	 * After the view file is identified, this method may further call {@link CApplication::findLocalizedFile}
	 * to find its localized version if internationalization is needed.
	 *
	 * @param mixed $layoutName layouts name
	 * @return string the view file for the layouts. False if the view file cannot be found
	 */
	...
	/**
	 * Renders a view with a layouts.
	 *
	 * This method first calls {@link renderPartial} to render the view (called content view).
	 * It then renders the layouts view which may embed the content view at appropriate place.
	 * In the layouts view, the content view rendering result can be accessed via variable
	 * <code>$content</code>. At the end, it calls {@link processOutput} to insert scripts
	 * and dynamic contents if they are available.
	 *
	 * By default, the layouts view script is "protected/views/layouts/main.php".
	 * This may be customized by changing {@link layouts}.
	 *
	 ...
	 * The string will be inserted in the current controller layouts and returned back.
	 ...
	 * apply a layouts to the rendered result. It is thus mostly used
